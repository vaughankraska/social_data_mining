...
def prepare_graph_for_betweenness(
    g: ig.Graph, base_size: int = 1, scale_factor: int = 5
):
    betweenness = g.betweenness()
    max_betweenness = max(betweenness) if max(betweenness) > 0 else 1

    g.vs["size"] = [
        base_size + scale_factor * (b / max_betweenness) for b in betweenness
    ]
...
def top_10_for_network(g: ig.Graph):
    ...
    top_10_degrees = dict(
        sorted(degree_dict.items(), key=lambda item: item[1], reverse=True)[:10]
    )

    betweenness = g.betweenness()
    betweenness_dict = {g.vs[i]["name"]: betweenness[i] for i in range(len(g.vs))}
    top_10_betweenness = dict(
        sorted(betweenness_dict.items(), key=lambda item: item[1], reverse=True)[:10]
    )
...
def random_diffusion(
    g: ig.Graph, actor: ig.Vertex = None, beta: float = 0.05, iterations: int = 100
) -> list:
    if actor is None:
        actor = g.vs[random.randint(0, g.vcount() - 1)]
    ...
    for _ in range(iterations):
        ...
        for infected_actor in infected_actors:
            ...
            infected_neighbors = random.choices(
                neighbors, weights=weights, k=len(neighbors)
            )
...
def random_opinion(
    g: ig.Graph, th: int, initial_positive: int, iterations: int = 100
) -> list:
    """
    ...
    starting_indices = random.choices(
        [i for i in range(g.vcount())], k=initial_positive
    )
